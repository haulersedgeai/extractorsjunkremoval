...
export default async function LocationPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <Hero
        eyebrow={isCity ? "Service Area" : "Neighborhood"}
        title={`Junk Removal in ${loc.fullName}`}
        subtitle={`Family-owned junk removal, cleanouts, and light demolition in ${loc.name}. Same-day pickup, upfront pricing, real five-star Google reviews.`}
      />
        ...
        <div className="grid gap-12 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <h2 className="font-display text-2xl font-extrabold text-ink sm:text-3xl">
              Serving {loc.fullName} ({loc.county})
            </h2>
      ...
      <Process steps={[
        { title: "Text us a photo", body: `From anywhere in ${loc.name}, send a pic of the pile. We'll quote you same-day.` },
        { title: "Pick a window", body: `We hold open spots every day for ${loc.name} customers.` },
        { title: "We arrive & haul", body: "Two-person uniformed crew, truck and dump trailer. You don't touch a thing." },
        { title: "Pay when done", body: "Final sweep, final price, payment when you're satisfied." },
      ]} />

      <WhyUs heading={`Why ${loc.name} Picks Extractors`} />
      <ReviewsStrip />
      <CTASection
